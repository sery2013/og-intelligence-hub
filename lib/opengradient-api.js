...
export async function getMemSyncData() {
  try {
    const response = await fetch('https://memsync.ai/api/user-stats')
    if (response.ok) return await response.json()
  } catch (error) {
    console.log('Using fallback MemSync data')
  }
  return {
    connectedSources: [
      { id: 1, name: 'Twitter/X', status: 'synced', lastSync: '5 min ago', items: 1247 },
      { id: 2, name: 'Google Drive', status: 'synced', lastSync: '1 hour ago', items: 89 },
      { id: 3, name: 'Notion', status: 'pending', lastSync: '-', items: 0 },
      { id: 4, name: 'Discord', status: 'error', lastSync: '2 days ago', items: 342 },
    ],
    stats: {
      totalItems: 1678,
      encryptedItems: 1678,
